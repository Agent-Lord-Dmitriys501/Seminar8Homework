...
int[,] Array4x4Spiralno()
{
    int n = 4;
    int[,] array4x4 = new int[n, n];

    int temp = 1;
    int i = 0;
    int j = 0;

    while (temp <= array4x4.GetLength(0) * array4x4.GetLength(1))
    {
        array4x4[i, j] = temp;
        temp++;
        if (i <= j + 1 && i + j < array4x4.GetLength(1) - 1)
            j++;
        else if (i < j && i + j >= array4x4.GetLength(0) - 1)
            i++;
        else if (i >= j && i + j > array4x4.GetLength(1) - 1)
            j--;
        else
            i--;
    }
